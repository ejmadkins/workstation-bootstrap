...
if [ "$MODE" = "cli" ]; then
    ...
    ssh -t "$SSH_HOST_ALIAS" "
        ...
        tmux has-session -t workstation 2>/dev/null && tmux attach-session -t workstation || {
            ...
            tmux send-keys -t workstation:0 'export PATH="\$HOME/.fzf/bin:\$PATH"; fzf_bin=\$(command -v fzf || echo "\$HOME/.fzf/bin/fzf"); selected=\$(find /home/user/projects /home/user -mindepth 1 -maxdepth 1 -type d 2>/dev/null | grep -F -v "/." | sort -u | \$fzf_bin --prompt=\"Select a Project > \" --height=40% --reverse --border); [ -n \"\$selected\" ] && cd \"\$selected\"; clear; agy' C-m && \
